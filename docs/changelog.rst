...
* **Breaking change!** The old frontend has been fully removed from the backend code. Updating to this version
...
Translations have moved from hosted.weblate.org to translate.codeberg.org.

Update notes
............
..
* Due to a search index backend change, the following must be run once:
...
* Add support for API versioning through the Accept header, to avoid potential breaking changes
  required by the SPA UI (WIP). Currently ``version=2.0`` is in use for the following:
    ...
    * The streams viewsets now prepend a context to the response payload when version == 2.0. This
      context is generated by the streams classes and currently includes the following
      properties:

      * ``notify_key``: the key from which the  websocket notifications url is derived.
      * ``unfetched_content``: True if the content ids fetched from the cache/DB exceeds the
        ``paginate_by`` value. Tells the new UI to use upper and lower boundaries when
        infinite scrolling triggers content fetches. More details to come in the new UI
        documentation.
...
* In support of richer AP profiles, these properties were added to users.Profile:
    ...
    * ``bio``: corresponds to the AP profile ``summary`` property.
    ...
    The new UI supports managing these new properties, along with ``name``.

* Federate the new profile properties (requires support from federation).

* In an effort to reduce page reloads for the new UI, the ``first_id`` stream query param was added.
  It tells the backend which content id is the first rendered content in the currently selected
  stream. This allows for a more efficient content loading in cases where there's a large amount
  of unfetched content.

* Use in conjunciton with ``first_id``, a request with the ``unfetched_count`` query parameter
  returns only the number of content with ids newer than ``first_id``. The new UI uses this to
  restore the actual new content count when switching stream tabs or after any kind of
  network outage.

* The Content serializer now returns the ``Content.slug`` property (for future use with the new UI)
  and ``notify_key`` which is derived from ``Content.channel_group_name`` to enable receiving relevant
  notifications when the new UI is showing a single content.

* Restore the ``children`` queryset to re-enable single reply thread fetching.

* The new UI websocket heartbeat implementation wants a pong reply to its pings. Added to
  StreamConsumer.receive.

* Implement a session authentication API.

* Implement a search API.

* Implement a media upload API, allowing audio, image and video medias.

* Implement per uuid API content fetches.
...
* Changes to handle the ``first_id`` query param were implemented in BaseStream.get_cached_content_ids and
...
* Update related objects after share retraction.

* Fix Diaspora reshare also being a content messing up the replies threads.

* Handle cases where base.Profile.raw_content is an array.
