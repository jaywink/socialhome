...
class TestGetFederableObject(SocialhomeTestCase):
    ...
    async def test_content_returned(self):
        request = RequestFactory().get(
            self.content.fid.replace(settings.SOCIALHOME_URL, ""),
            HTTP_HOST="127.0.0.1:8000",
        )
        content = await get_federable_object(request)
    ...
    async def test_content_returned__site_content(self):
        request = RequestFactory().get(
    ...
    async def test_content_not_returned__site_content_anonymous_user(self):
        request = RequestFactory().get(
            self.site_content.fid.replace(settings.SOCIALHOME_URL, ""),
            HTTP_HOST="127.0.0.1:8000",
        )
    ...
    async def test_profile_not_returned__site_user_anonymous_user(self):
        request = RequestFactory().get(
            self.site_profile.fid.replace(settings.SOCIALHOME_URL, ""),
            HTTP_HOST="127.0.0.1:8000",
        )
        profile = await get_federable_object(request)
        self.assertIsNone(profile)

    async def test_profile_returned(self):
        request = RequestFactory().get(
    ...
    async def test_profile_returned__site_user(self):
        request = RequestFactory().get(
            self.site_profile.fid.replace(settings.SOCIALHOME_URL, ""),
            HTTP_HOST="127.0.0.1:8000",
        )
    ...
    async def test_profile_returned_for_root_profile(self):
...
class TestGetProfile(SocialhomeTestCase):
    ...
    async def test_profile_returned(self):
        returned = await get_profile(fid=self.profile.fid)
